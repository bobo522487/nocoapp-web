...
const Header: React.FC<HeaderProps> = ({ isDarkMode, toggleTheme }) => {
  ...
  // Helper to map route to legacy ViewMode for Breadcrumb
  const getActiveView = (path: string): ViewMode => {
    if (path.startsWith('/apps')) return ViewMode.APPS;
    if (path.startsWith('/data')) return ViewMode.DATA;
  ...
  return (
    <header className="h-14 flex items-center justify-between px-4 border-b border-border bg-card shrink-0 z-30 transition-colors">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Breadcrumb */}
        <Breadcrumb activeView={getActiveView(location.pathname)} />
      </div>
