...
import { useEditorStore, selectActiveFileContent } from '../../stores/useEditorStore';

const Editor: React.FC = () => {
  const { tabs, activeTabId, closeTab, setActiveTab, updateFileContent } = useEditorStore();
  
  // Use a selector-like pattern or just access state directly
  const activeFileContent = useEditorStore(selectActiveFileContent);
  ...
  const handleContentChange = (content: string) => {
    ...
    if (activeTab) {
      updateFileContent(activeTab.fileId, content);
    }
  };
